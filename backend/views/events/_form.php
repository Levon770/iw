...
<div class="span3">
	...
	<?php if(!empty($model->image)) echo Chtml::image('/iw/frontend/www/userfiles/events/'.$model->folder.'/'.$model->image,'',array('width'=>'150'))?>
	...
	<?php echo $form->textAreaRow($model,'video',array('rows'=>6, 'cols'=>50, 'class'=>'span3', 'hint'=>'youtube link')); ?>
        ...
        <?php if(!empty($model->gallery)){
        	...
        	foreach ($img as $item){
        		echo CHtml::Image('/iw/frontend/www/userfiles/events/'.$model->folder.'/thumbs/'.$item,'',array('width'=>'80', 'style'=>'margin:5px 10px'));
        	}
        	}?>

        <!-- DUE DATE-->
	<?php echo $form->datepickerRow($model, 'timestamp', 
    array(
      'size'=>60,
      'maxlength'=>95, 
      'label'=>false,
      'class'=>'input-medium span3', 
