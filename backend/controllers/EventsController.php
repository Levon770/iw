...
$model->timestamp = strtotime($model->timestamp); 
//var_dump($model->timestamp);echo "<br/>"; var_dump(strtotime('now'));exit;

        $path = $this->randstring(); $model->folder = $path;
        $thumbpath = $path.'/thumbs';
        mkdir(getcwd().'/frontend/www/userfiles/events/'.$path);  
        mkdir(getcwd().'/frontend/www/userfiles/events/'.$path.'/thumbs');
...
if($model->save()){ 
      if(!empty($logoUpload)){ 
        $logoUpload->saveAs(Yii::app()->basePath.'/../frontend/www/userfiles/events/'.$path.'/'.$fileNameImage); 
        $logo= Yii::app()->image->load(Yii::app()->basePath.'/../frontend/www/userfiles/events/'.$path.'/'.$fileNameImage);
		    $logo->resize(400, 400);
		    $logo->save();
      } 
...
{
	$model->attributes=$_POST['Events'];
	$_POST['Events']['image'] = $model->image;
		$logoUpload=CUploadedFile::getInstance($model,'image');
    //Uploading gallery images
        if (!empty($gallery) !=NULL){
                            if($filez=$this->uploadMultifile($model,'gallery',$path ,500,500));
...
if($model->save()){ 
      if(!empty($logoUpload)){ 
        $logoUpload->saveAs(Yii::app()->basePath.'../frontend/www/userfiles/events/'.$model->folder.'/'.$model->image); 
        $logo= Yii::app()->image->load(Yii::app()->basePath.'/../frontend/www/userfiles/events/'.$model->folder.'/'.$model->image);
		$logo->resize(400, 400);
		$logo->save();
      } 
    $this->redirect(array('view','id'=>$model->id)); 
...
public function uploadMultifile ($model,$attr,$path, $w, $h)
      ...
      foreach ($sfile as $i=>$files){  
        ...
        $filepath=Yii::app()->basePath.'/../frontend/www/userfiles/events/'.$path.'/'.$formatName;
